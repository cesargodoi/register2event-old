...
def update_mapping(evenid):
    ...
    buildings = current.db(
        (current.db.building.center == event.center)
        & (current.db.building.is_active == True)
    ).select()
    mapping = (
        current.db(current.db.bedrooms_mapping.evenid == evenid)
        .select()
        .first()
    )
    if mapping:
        ...
        for building in buildings:
            _ids = [
                int(b.id) for b in building.bedroom.select() if b.is_active
            ]
        ...
        if to_add:
            bedrooms = current.db(
                current.db.bedroom.id.belongs(to_add)
            ).select()
            bedrooms_to_add = []
            for b in bedrooms:
                if b.is_active:
        ...
        current.db.bedrooms_mapping.insert(
            centid=event.center, evenid=event.id, bedrooms=bedrooms
        )
...
def attempt_deallocation(br, mapping, event):
    ...
    if to_deallocation:
        for g in to_deallocation:
            ...
            guest_stay = (
                current.db(
                    (current.db.guest_stay.guesid == g)
                    & (current.db.guest_stay.center == event.center)
                )
                .select()
                .first()
            )
...
def unallocateds(evenid):
    ...
    query = (
        (current.db.register.evenid == evenid)
    ...
    regs = [
        reg
        for reg in current.db(query).select()
        if not reg.credit and not reg.bedroom
    ]
...
def unallocateds_by_gender(gender):
    if gender == "X":
        ...
        _unallocateds = [
            un
            for un in current.session.mapp.unallocateds
            if un["gender"] == gender
        ]
...
def compare_mapping_and_bedrooms(mapp):
    mapping = [int(mp.id) for mp in mapp]
    bedrooms = [
        int(bed.id)
        for bed in current.db(
            (current.db.bedroom.id > 0)
            & (current.db.bedroom.is_active == True)
        ).select()
    ]
    mapping.sort()
    bedrooms.sort()
...
def add_mapp(_id):
    return [
        [mp[1], mp[2]] for mp in current.session.mapp.mapping if mp[0] == _id
    ][0]
...
def gen_mapp_buildings(rows):
    ...
    for row in rows:
        ...
        in_use += len([bed for bed in row.mapp[0] if bed != 0]) + len(
            [top for top in row.mapp[1] if top != 0]
        )
...
def gen_mapp_building(rows):
    ...
    for row in rows:
        ...
        beds_in_use += len([bed for bed in row.mapp[0] if bed != 0])
        tops_in_use += len([top for top in row.mapp[1] if top != 0])
    return (
        bedrooms,
        total_beds,
        beds_in_use,
...
def gen_mapp_gender(row, mapp):
    return (
        len([bed for bed in mapp[0] if bed != 0]),
        len([top for top in mapp[1] if top != 0]),
    )
...
def choose_a_bed(evenid, guesid, bedroomid, regid, from_mapp):
    # select event
    event = current.db(current.db.events.id == evenid).select().first()
    # select guest stay
    guest_stay = (
        current.db(
            (current.db.guest_stay.guesid == guesid)
            & (current.db.guest_stay.center == event.center)
        )
        .select()
        .first()
    ...
    # select bedroom
    bedroom = [bed for bed in mapping.bedrooms if bed[0] == bedroomid][0]
    # attempt
    attempt = False
    ...
    if attempt:
        ...
        if event.activity.activity_type == "SCF":
...
def put_on_a_bedroom(guest, bedrooms):
    ...
    for bedroom in bedrooms:
        if guest["no_top_bunk"] or guest["age"] >= 60:
            if 0 in bedroom[1]:
                bedroom[1].remove(0)
