...
def hooks():
    import functools
    import inspect
    list_op = ['_%s_%s' % (h, m) for h in ['before', 'after']
                    ...
                    if hasattr(f, '__call__'):
                        try:
                            ...
                            if details['filename']:  # Built in functions as delete_uploaded_files are not editable
                                details['url'] = URL(a='admin', c='default', f='edit', args=[
                                                     request['application'], details['filename']], vars={'lineno': details['lineno']})
    ...
    for t in tables:
        ...
        for op in t['method_hooks']:
            ...
            ul_t.append(UL([LI(A(f['funcname'], _class="editor_filelink", _href=f['url']
                                 if 'url' in f else None, **{'_data-lineno': f['lineno'] - 1})) for f in op['functions']]))
        ul_main.append(ul_t)
    return ul_main
