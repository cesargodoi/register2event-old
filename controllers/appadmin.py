...
if request.function == "manage":
    ...
    and not gluon.fileutils.check_credentials(request)
):
    redirect(
        URL(
            "admin",
...
def csv():
    ...
    response.headers[
        "Content-disposition"
    ] = "attachment; filename=%s_%s.csv" % tuple(
...
def update():
    ...
    if keyed:
        ...
        if key:
            record = (
                db(db[table][key[0]] == request.vars[key[0]]).select().first()
            )
    else:
...
def ccache():
    ...
    if is_gae:
        ...
        try:
            total["ratio"] = (
                total["hits"] * 100 / (total["hits"] + total["misses"])
            )
        except (KeyError, ZeroDivisionError):
            total["ratio"] = 0
    ...
    return dict(
        form=form, total=total, ram=ram, disk=disk, object_stats=hp != False
    )
...
def table_template(table):
    ...
    return (
        ...
        % TABLE(
            *rows,
            **dict(_bgcolor=bgcolor, _border=1, _cellborder=0, _cellspacing=0)
        ).xml()
    )
