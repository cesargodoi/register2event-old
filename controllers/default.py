...
def index():
    ...
    if session.mapp or session.register:
        clear_session()
    query = (
        (Events.id > 0) & (Events.status == "OPN") & (Events.is_active == True)
    )
    ...
    return dict(message=T("Welcome to reg2event!"), rows=rows, page="index")
...
def user():
    ...
    form.element(_type="submit")[
        "_class"
    ] = "btn btn-outline btn-primary btn-lg btn-block"
...
def manage_guest():
    response.view = "default/manage.html"
    grid = SQLFORM.grid(Guest, orderby=[Guest.name_sa], showbuttontext=False)
    return dict(grid=grid)
...
def manage_building():
    response.view = "default/manage.html"
    grid = SQLFORM.grid(
        Building, orderby=[Building.building], showbuttontext=False
    )
    return dict(grid=grid)
...
def manage_bedroom():
    response.view = "default/manage.html"
    grid = SQLFORM.grid(
        Bedroom, orderby=[Bedroom.builid], showbuttontext=False
    )
    return dict(grid=grid)
...
def manage_activities():
    response.view = "default/manage.html"
    grid = SQLFORM.grid(
        Activities, orderby=[Activities.activity_type], showbuttontext=False
    )
    return dict(grid=grid)
...
def manage_events():
    response.view = "default/manage.html"
    grid = SQLFORM.grid(
        Events, orderby=[Events.center, Events.activity], showbuttontext=False
    )
    return dict(grid=grid)
...
def manage_bank_flag():
    response.view = "default/manage.html"
    grid = SQLFORM.grid(
        Bank_Flag, orderby=[Bank_Flag.name], showbuttontext=False
    )
    return dict(grid=grid)
...
def manage_payment_form():
    ...
    grid = SQLFORM.grid(
        Payment_Form,
        orderby=[Payment_Form.bank_flag, Payment_Form.created_on],
        showbuttontext=False,
    )
    return dict(grid=grid)
...
def manage_guest_stay():
    response.view = "default/manage.html"
    grid = SQLFORM.grid(
        Guest_Stay, orderby=[Guest_Stay.id], showbuttontext=False
    )
    return dict(grid=grid)
...
def change_center():
    response.view = "default/manage.html"
    CENTERS = {
        c.id: center_repr % c for c in db(Center).select(orderby=Center.id)
    }
    grid = SQLFORM.factory(
        Field(
            "center", requires=IS_IN_SET(CENTERS), default=1, label=T("center")
        ),
...
def kill_them_all():
    ...
    if search.process().accepted:
        ...
        if read:
            ...
            session.kill_register.key = str(
                CRYPT(digest_alg="sha1", salt=False)(request.vars.number)[0]
            )
    ...
    return dict(search=search, info=False)
...
def kill_register():
    ...
    if (
        str(CRYPT(digest_alg="sha1", salt=False)(request.vars.number)[0])
        != session.kill_register.key
    ):
        redirect(URL("default", "index"))
    ...
    if session.kill_register.registers:
        registers = db(
            Register.id.belongs(session.kill_register.registers)
        ).delete()

    if session.kill_register.payforms:
        payforms = db(
            Payment_Form.id.belongs(session.kill_register.payforms)
        ).delete()
...
def adj_bedroom_alt():
    ...
    EVENTS = {
        ev.id: event_repr(ev) % ev
        for ev in db(
            (Events.center == 2) & (Events.activity.belongs(SCF))
        ).select(orderby=~Events.id)
    }
    form = SQLFORM.factory(
        Field("event", requires=IS_IN_SET(EVENTS), label=T("event")),
    ...
    if form.process().accepted:
        ...
        for reg in registers:
            last_stay = [
                sty for sty in stays_on_center if sty.guesid == reg.guesid
            ]
            if last_stay and last_stay[0].bedroom_alt != reg.bedroom:
                _stay = (
                    db(
                        (Guest_Stay.center == 2)
                        & (Guest_Stay.guesid == reg.guesid)
                    )
                    .select()
                    .first()
                )
